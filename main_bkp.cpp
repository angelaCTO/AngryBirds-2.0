...
#define FPS                20
#define X_RESOLUTION       352  /* why too low? */
...
int main()
{
  /*---------------------------------------------------
    ...
    ofstream signals, ab_log;

    // Gets analog readings from adc pin 4      /* no need */
    BlackADC *test_adc = new BlackADC(AIN4); 	       // P9_34/33
    BlackGPIO *ir_in = new BlackGPIO(GPIO_68, input);  // P8_10

    ab_log.open("/home/ubuntu/AngryBirds/SDCard/log.txt",
                          fstream::in  |
                          fstream::out |
                          fstream::app);
    ab_log << get_date() << " - program started." << endl;
// int tester = 0; for testing ...can delete
  /*---------------------------------------------------
    ...
    if(rc){
        ab_log << get_date() << " - ERROR: unable to create thread." << endl;
    }
    ...
    ab_log << get_date() << " - CREATING MAIN IMAGES DIRECTORY." << endl;
    // Create the (final) output destination - where all concatenated
    // clips will be stored ("Images" directory)
    path_name = create_dir_path(img_path, "NONE"); /* ask angela */
    converted_path = path_name.c_str();
    create_directory(converted_path, st);
    ab_log << get_date() << " - DONE CREATING MAIN VIDEOS DIRECTORY: " << converted_path << endl;

    ab_log << get_date() << " - CREATING MAIN VIDEOS DIRECTORY" << endl;
    // Create the (final) output destination - where all concatenated
  ...
  /*---------------------------------------------------
    ...
    while(true){
        ...
        if (!input_cap.isOpened()) {
            ab_log << get_date() << " - INPUT VIDEO COULD NOT BE OPENED!" << endl;
            sleep(1);
            return -1;
        }
